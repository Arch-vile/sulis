...
<g:form action="create">
	...
	<g:select
		name="${prop(of(CreateGameCommand.class).game.servingPlayer)}"
		from="${players}"
		optionKey="name"
		optionValue="name" />
		
	<br/>
	or new  one <g:textField 
		name="${prop(of(CreateGameCommand.class).newServingPlayer) }"
		value="${fieldValue(bean: createGameCommand, field: 'newServingPlayer')}"
		/>

	<g:select 
		name="${prop(of(CreateGameCommand.class).game.receivingPlayer)}" 
		from="${players}" 
		optionKey="name" 
		optionValue="name" />
		
	<br/>
	or new  one <g:textField name="${prop(of(CreateGameCommand.class).newReceivingPlayer) }"/>
	
	<br/>
	receiving score: 
	<g:select 
		name="${prop(of(CreateGameCommand.class).game.servingPlayerPoints) }"
		from="${[*21..0, *22..25]}" />
	
	<br/>
	serving score: 
	<g:select 
		name="${prop(of(CreateGameCommand.class).game.receivingPlayerPoints) }"
		from="${[*21..0, *22..25]}" />
